        ...
        header.addEventListener("click", function() {
            ...
            rows.sort((row1, row2) => {
                ...
                if (columnIndex === 3 || columnIndex === 6) {
                    ...
                    return (sortOrder[columnIndex] === "asc" ? 1 : -1) * (numValue1 - numValue2);
                }

                // Sort by custom order if sorting formality
                if (columnIndex === 4) {
                    const formalityOrder = ["Very Formal", "Formal", "Casual", "Very Casual"];
                    const index1 = formalityOrder.indexOf(cell1.innerHTML);
                    const index2 = formalityOrder.indexOf(cell2.innerHTML);
                    return (sortOrder[columnIndex] === "asc" ? 1 : -1) * (index1 - index2);
                }

                // Sort by alphabetical order by default
                return (sortOrder[columnIndex] === "asc" ? 1 : -1) * cell1.innerHTML.localeCompare(cell2.innerHTML);
            });
